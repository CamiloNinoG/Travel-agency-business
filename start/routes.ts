...
// Drivers
Route.group(() => {
  Route.get("/", "DriversController.find"); // Lista todos los drivers o paginados
  Route.get("/:id", "DriversController.find"); // Trae un driver específico
  Route.post("/", "DriversController.create"); // Crear nuevo driver
  Route.put("/:id", "DriversController.update"); // Actualizar driver
  Route.delete("/:id", "DriversController.delete"); // Eliminar driver
}).prefix("/drivers");

// Shifts
Route.group(() => {
  Route.get("/", "ShiftsController.find"); // Lista todos los shifts o paginados
  Route.get("/:id", "ShiftsController.find"); // Trae un shift específico
  Route.post("/", "ShiftsController.create"); // Crear nuevo shift
  Route.put("/:id", "ShiftsController.update"); // Actualizar shift
  Route.delete("/:id", "ShiftsController.delete"); // Eliminar shift
}).prefix("/shifts");


//enviar mensaje alert
Route.group(() => {
  // Enviar alerta climática a todos los conductores
  Route.post("/drivers/alert-weather", "DriversController.alertWeather");
}).prefix("/api");
